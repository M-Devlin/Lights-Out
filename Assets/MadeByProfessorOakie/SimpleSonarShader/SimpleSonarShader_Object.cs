...
public class SimpleSonarShader_Object : MonoBehaviour
{
    ...
    public void StartSonarRing(Vector4 position, float intensity)
    {
        ...
        position.w = Time.time; //current time in-game. Specifically, the total amount of time the game has been running.
    ...
    private void SendSonarData()
    {
        ...
        {
            renderer.material.SetVectorArray("_hitPts", positionsQueue.ToArray());
            renderer.material.SetFloatArray("_Intensity", intensityQueue.ToArray());
        }
    }
    ...
    void Update()
    {
        ...
        {
            //Debug.Log("boop");
            StartSonarRing(Player.transform.position, 10.0f);
        }
